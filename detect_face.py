...
def create_mtcnn(sess, model_path):
    ...
    with tf.variable_scope('pnet'):
        ...
        pnet = PNet({'data': data})
        pnet.load(os.path.join(model_path, 'weight/det1.npy'), sess)
    with tf.variable_scope('rnet'):
        data = tf.placeholder(tf.float32, (None, 24, 24, 3), 'input')
        rnet = RNet({'data': data})
        rnet.load(os.path.join(model_path, 'weight/det2.npy'), sess)
    with tf.variable_scope('onet'):
        data = tf.placeholder(tf.float32, (None, 48, 48, 3), 'input')
        onet = ONet({'data': data})
        onet.load(os.path.join(model_path, 'weight/det3.npy'), sess)
...
def bulk_detect_face(images, detection_window_size_ratio, pnet, rnet, onet, threshold, factor):
    ...
    for resolution in images_obj_per_resolution:
        ...
        for index in range(len(outs[0])):
            ...
            if boxes.size > 0 and pick.size > 0:
                boxes = boxes[pick, :]
                images_with_boxes[image_index]['total_boxes'] = np.append(images_with_boxes[image_index]['total_boxes'], boxes, axis=0)
